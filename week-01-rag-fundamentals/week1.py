...
class LocalVectorStore:
    ...
    def retrieve(
        ...
        if diversity > 0:
            idx = self._mmr(sims, top_k, diversity)
        else:
            k = min(top_k, sims.shape[0])
            idx = np.argpartition(-sims, k - 1)[:k]
            idx = idx[np.argsort(-sims[idx])]

        return [(self.records[i]["metadata"], float(sims[i])) for i in idx]

    def _mmr(self, sims: np.ndarray, top_k: int, lambda_mult: float) -> List[int]:
        k = min(top_k, sims.shape[0])
        candidates = list(range(sims.shape[0]))
        selected: List[int] = []
        ...
        while len(selected) < k and candidates:
            if not selected:
                ...
                sel_mat = self._matrix[selected]
                best, best_score = -1, -np.inf
                for i in candidates:
                    relevance = float(sims[i])
                    redundancy = float(np.max(sel_mat @ self._matrix[i]))
...
def build_context(results: List[Tuple[Dict[str, Any], float]]) -> str:
    blocks: List[str] = []
    used = 0

    for chunk, _score in results:
        block = (
            f"[Source {chunk['chunk_id']} | {chunk['source']} p.{chunk['page']}]\n"
            f"{chunk['text']}"
        )
...
with st.sidebar:
    ...
    diversity = st.slider("Diversity (MMR)", 0.0, 1.0, 0.0, step=0.1)
...
with header_left:
    st.markdown(
        """
        <div>
            <div class="app-title">Local RAG</div>
            <div class="app-subtitle">Private retrieval-augmented chat over your documents</div>
        </div>
        """,
...
if process_clicked:
    if not uploaded_files:
        ...
        if not all_chunks:
            ...
            progress = st.progress(0.0, text="Generating embeddings...")
        ...
        st.markdown(
            ...
            <div class="source-card">
                <div class="source-head">
                    <div class="source-title">{s['source']} &middot; page {s['page']} &middot; chunk {s['chunk_id']}</div>
                    <div class="source-score">{s['score']:.3f}</div>
                </div>
                <div class="source-body">{s['text'][:700]}{'…' if len(s['text']) > 700 else ''}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        with st.chat_message("assistant"):
            with st.spinner("Retrieving context..."):
                try:
                    ...
                    results = st.session_state.vector_store.retrieve(
                        query_vec, top_k=top_k, diversity=diversity
                    )
                except Exception as exc:
